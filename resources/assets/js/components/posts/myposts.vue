          ...
          <div class="col-lg-4 col-md-4 col-sm-8 col-xs-12" v-for="(post,index) in posts">
            <div class="thumbnail" id="post_thumbnail">
              ...
              <div class="btn_more_details">
                 ...
                 <a  v-if="$auth.check() && post.user_id == $auth.watch.data && !post.is_published"  class="btn btn-success post_read_more_btn" role="button" @click="publishPost(post.id)">
                       Publier
                 </a>
...
import Swal from "sweetalert2"
    ...
    methods: {
      ...
      loadCategories() {
        ...
        loadPosts(page) {
                 axios.get('/LoadMyPosts?page='+page).then((response) => {
        ...
        publishPost(id){
          axios.post('/publishpost',{postId:id})
                  .then(resp => {
                       Swal.fire({  
                        title: 'Publier post', 
                        text: 'Vote post a été publié avec succés', 
                        icon: 'success', 
                        confirmButtonText: 'Fermer', 
                    });
                  })
